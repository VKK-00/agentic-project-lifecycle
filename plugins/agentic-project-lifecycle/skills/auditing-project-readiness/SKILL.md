...
- **RULE-AUDIT-01:** Run the relevant bundled validators, including `validate_task_contract.py`, `validate_gate_transition.py`, `validate_evidence.py`, and strict project-state validation, before stating readiness. Report every blocking condition with the artifact, owner, and expected correction.
...
- **RULE-AUDIT-04:** Collect source-bound evidence with the full source commit, timezone-aware timestamps, command exit codes, durations, environment, logs, and artifact digests. Never replace observed evidence with an agent statement that checks passed.
- **RULE-AUDIT-05:** Block release readiness when an open hard blocker exists or when owner, support, telemetry, rollout, rollback, migration, restore, source-bound verification, or critical-risk evidence required by the release stage is missing.
